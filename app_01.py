...
# Load GCN-LSTM model and label mapping
try:
    model = GCN_LSTM_Model().to(DEVICE)
    model.load_state_dict(torch.load(MODEL_PATH, map_location=DEVICE))
    model.eval()
    logging.info("GCN-LSTM model loaded successfully")
except Exception as e:
    logging.error(f"Failed to load GCN-LSTM model: {str(e)}")
    raise

try:
    with open(LABEL_MAP_PATH, "rb") as f:
        label_map = pickle.load(f)
    logging.info(f"Label map loaded: {label_map}")
except Exception as e:
    logging.error(f"Failed to load label map: {str(e)}")
    raise
...
logging.info(f"Action to name mapping: {action_to_name}")

# Skeleton connections (COCO format for VitPose)
...
def generate_frames():
    ...
    while True:
        ...
        logging.debug(f"Frame {ith_img}: FPS = {fps:.2f}")
        cv2.putText(img, f"FPS: {fps:.2f}", (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        ...
        try:
            ...
            logging.debug(f"Detected {len(person_boxes_xyxy)} persons")
            ...
            inputs = pose_processor(images=rgb, boxes=[person_boxes_xywh], return_tensors="pt").to(DEVICE)
            with torch.no_grad():
            ...
            for i, pose_result in enumerate(results_pose):
                ...
                logging.debug(f"Person {i+1}: Keypoints shape: {keypoints_with_conf.shape}, sample: {keypoints_with_conf[:2]}")
                bbox = person_boxes_xyxy[i]
                logging.debug(f"Person {i+1}: BBox: {bbox}")
                person_id, prev_centroids = assign_id(bbox, prev_centroids)

                norm_keypoints = normalize_keypoints(keypoints_with_conf, bbox)
                logging.debug(f"Person {person_id}: Normalized keypoints sample: {norm_keypoints[:2]}")
                if np.any(np.isnan(norm_keypoints)):
                    logging.warning(f"Person {person_id}: NaN detected in normalized keypoints")
                    continue

                sequence_buffer.append(norm_keypoints.flatten())
                logging.debug(f"Person {person_id}: Sequence buffer length: {len(sequence_buffer)}")

                action_label = "Unknown"
                if len(sequence_buffer) >= sequence_length:
                    input_data = np.array(sequence_buffer[-sequence_length:]).reshape(1, sequence_length, num_keypoints * 3)
                    logging.debug(f"Input data shape: {input_data.shape}, sample: {input_data[0, 0, :6]}")
                    if np.isnan(input_data).any():
                        logging.warning(f"Person {person_id}: NaN in input data, skipping prediction")
                    ...
                    with torch.no_grad():
                        ...
                        logging.debug(f"Person {person_id}: Model prediction logits: {prediction[0]}")
                        predicted_class = torch.argmax(prediction, dim=1).item()
                        logging.debug(f"Person {person_id}: Predicted class index: {predicted_class}")
                        try:
                            action_xx = sorted(list(set(label_map.values())))[predicted_class]
                            action_label = action_to_name.get(action_xx, "Unknown")
                            logging.info(f"Person {person_id}: Predicted action: {action_label}")
                        except IndexError:
                            logging.error(f"Person {person_id}: Invalid class index {predicted_class}, label_map size: {len(set(label_map.values()))}")
                            action_label = "Unknown"
                ...
                cv2.putText(img, text, (10, 40 + (int(person_id)-1)*30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2)
...
def generate_video_predict_frames():
    ...
    if not video_file_path or not os.path.exists(video_file_path):
        logging.error("No video file path provided or file does not exist")
        placeholder = np.zeros((frame_height, frame_width, 3), dtype=np.uint8)
        text = "No Prediction"
        font = cv2.FONT_HERSHEY_TRIPLEX
        font_scale = 1.0
    ...
    orig_height = int(cap.get(cv2.CAP_PROP_HEIGHT))
    ...
    if video_predict_writer is None:
    ...
    while cap.isOpened():
        ...
        logging.debug(f"Video frame {ith_img+1}: Progress = {video_predict_progress:.1f}%")
        ...
        try:
            ...
            for box in results.boxes.data:
                ...
                if int(cls.item()) == 0:
                    person_boxes_xyxy.append([x1.item(), y1.item(), x2.item(), y2.item()])
            logging.debug(f"Video frame {ith_img+1}: Detected {len(person_boxes_xyxy)} persons")
            ...
            for i, pose_result in enumerate(results_pose):
                ...
                scores = pose_result["scores"].cpu().numpy()
                keypoints_with_conf = np.concatenate([keypoints, scores[..., None]], axis=-1)
                logging.debug(f"Video frame {ith_img+1}, Person {i+1}: Keypoints shape: {keypoints_with_conf.shape}, sample: {keypoints_with_conf[:2]}")
                bbox = person_boxes_xyxy[i]
                logging.debug(f"Video frame {ith_img+1}, Person {i+1}: BBox: {bbox}")
                person_id, prev_centroids = assign_id(bbox, prev_centroids)

                norm_keypoints = normalize_keypoints(keypoints_with_conf, bbox)
                logging.debug(f"Video frame {ith_img+1}, Person {person_id}: Normalized keypoints sample: {norm_keypoints[:2]}")
                if np.any(np.isnan(norm_keypoints)):
                    logging.warning(f"Video frame {ith_img+1}, Person {person_id}: NaN detected in normalized keypoints")
                    continue

                video_sequence_buffer.append(norm_keypoints.flatten())
                logging.debug(f"Video frame {ith_img+1}, Person {person_id}: Sequence buffer length: {len(video_sequence_buffer)}")

                action_label = "Unknown"
                if len(video_sequence_buffer) >= sequence_length:
                    input_data = np.array(video_sequence_buffer[-sequence_length:]).reshape(1, sequence_length, num_keypoints * 3)
                    logging.debug(f"Video frame {ith_img+1}, Person {person_id}: Input data shape: {input_data.shape}, sample: {input_data[0, 0, :6]}")
                    if np.isnan(input_data).any():
                        logging.warning(f"Video frame {ith_img+1}, Person {person_id}: NaN in input data, skipping prediction")
                    ...
                    with torch.no_grad():
                        prediction = model(input_tensor, edge_index)
                        logging.debug(f"Video frame {ith_img+1}, Person {person_id}: Model prediction logits: {prediction[0]}")
                        predicted_class = torch.argmax(prediction, dim=1).item()
                        logging.debug(f"Video frame {ith_img+1}, Person {person_id}: Predicted class index: {predicted_class}")
                        try:
                            action_xx = sorted(list(set(label_map.values())))[predicted_class]
                            action_label = action_to_name.get(action_xx, "Unknown")
                            logging.info(f"Video frame {ith_img+1}, Person {person_id}: Predicted action: {action_label}")
                        except IndexError:
                            logging.error(f"Video frame {ith_img+1}, Person {person_id}: Invalid class index {predicted_class}, label_map size: {len(set(label_map.values()))}")
                            action_label = "Unknown"
...
def register():
    ...
    if request.method == 'POST':
        ...
        if User.query.filter_by(username=username).first():
            logging.error(f"Registration failed: Username {username} exists")
...
def upload_video():
    ...
    try:
        with open(temp_file_path, mode) as f:
            file.save(f)
        logging.debug(f"Chunk {chunk_index+1}/{total_chunks} saved to {temp_file_path}")
    except Exception as e:
        logging.error(f"Error saving chunk: {str(e)}")
        return {"status": "error", "message": "Failed to save chunk"}, 500

    if chunk_index + 1 == total_chunks:
        mime_type, _ = mimetypes.guess_type(temp_file_path)
        if mime_type not in allowed_mime_types:
            os.unlink(temp_file_path)
            logging.error(f"Invalid file type: {mime_type}")
...
def predict_video():
    ...
    logging.info("Video prediction started")
    return {"status": "success"}
...
def save_video_result():
    ...
    logging.info(f"Saving predicted video: {filename}")
...
def save_settings():
    ...
    if sensitivity in ['low', 'medium', 'high'] and isinstance(time, int) and time >= 1:
        ...
        return {"status": "success"}
    logging.error(f"Invalid settings: sensitivity={sensitivity}, time={time}")
